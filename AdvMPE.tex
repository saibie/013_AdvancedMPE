...
\begin{lemma}{\rm \cite[Lemma 3.3]{SeoSeoKim2018}}\label{lem:TaylorLemma}
	...
	\begin{equation}
		\| P_{\e}({X}_{i})\|\leq a\|\tilde{X}_{\e,i}\|^{2}+b\|\tilde{X}_{\e,i}\|\|\tilde{X}_{\e,i-1}\|+c\|\tilde{X}_{\e, i-1}\|^{2}
	\end{equation}
...
%\end{proof}
In Lemma \ref{lem:TaylorLemma}, the equation is different from that in \cite[Lemma 3.3]{SeoSeoKim2018}.
But, in the same way, Lemma \ref{lem:TaylorLemma} can be proved, easily.
...
\begin{proof}
	...
	\begin{align*}
		P(X_{\e,i}) &= P(S) + P'_{\e}[S](\tilde{X}_{\e,i}) + O(\|\tilde{X}_{\e,i}\|^{2}) \\
		&= \e\pjn(\tilde{X}_{\e,i}) + P'_{\e}[S]\left(\pjm(\tilde{X}_{\e,i})\right) + O(\|\tilde{X}_{\e,i}\|^{2}). \numberthis\label{eq:TaylorMPE}
	\end{align*}
	Since $P'_{\e}[S]|_{\M} : \M \rightarrow \M$ is invertible, $\left\| P'_{\e}[S]\left(\pjm(\tilde{X}_{\e,i})\right)\right\| \geq c_{1}\|\pjm(\tilde{X}_{\e,i})\|$ for some constant $c_{1}>0$.
	...
	\|P(X_{i})\| \geq c_{1}\|\pjm(\tilde{X}_{i})\| - c_{2}\|\tilde{X}_{i}\|^{2} \geq c_{1}(\theta^{-1}+1)^{-1}\|\tilde{X}_{i}\| - c_{2}\|\tilde{X}_{i}\|^{2}.
	\end{equation}
	On the other hand, from Lemma \ref{lem:TaylorLemma}, we have
	$$
	\| P({X}_{i})\|\leq c_{3}\|\tilde{X}_{i}\|^{2}+c_{4}\|\tilde{X}_{i-1}\|\|\tilde{X}_{i}\|+c_{5}\|\tilde{X}_{i-1}\|^{2}.
	$$
	From \eqref{eq:NormofhatXi} and the fact that $X_{i}\neq S$ for
	any $i$, we have
	$$
	c_{1}(\theta^{-1}+1)^{-1} - c_{2}\|\tilde{X}_{i}\| \leq c_{3}\|\tilde{X}_{i}\| + c_{4}\|\tilde{X}_{i-1}\| + c_{5}\frac{\|\tilde{X}_{i-1}\|^{2}}{\|\tilde{X}_{i}\|}.
	$$
	Since $\tilde{X}_{i}$ converges to $0$ by Theorem \ref{thm:ConvergenceNewtonMPE},
	we can find an $i_{0}$ such that $\|\tilde{X}_{i}\|\leq c\|\tilde{X}_{i-1}\|^{2}$
	for all $i\geq i_{0}$.
\end{proof}
...
\begin{equation}
	-P'_{\e}[S](\M_{\e}) = \M_{\e},
\end{equation}
if $\RR^{\mbym} = \N_{\e} \oplus \M_{\e}$.
Moreover, we can define $\proj_{\N_{\e}}$ to be the projection onto $\N_{\e}$ parallel to $\M_{\e}$ and $\proj_{\M_{\e}}=I-\proj_{\N_{\e}}$.
...
\begin{theorem}\label{thm:SingularConvergence}
	Let $\proj_{\N_{0}} (P_{0}''[S](N, N))$ be nonzero and let
	\begin{equation}\label{eq:Wset}
	W(\rho, \theta) = \left\{ X \left|\,0 < \|\tilde{X}\| < \rho, ~\|\proj_{\M_{0}}(\tilde{X})\| \leq \theta \|\proj_{\N_{0}}(\tilde{X})\|\right.\right\}.
	\end{equation}
	If $X_{0}\in W(\rho_{0}, \theta_{0})$ for $\rho_{0},\theta_{0}$ sufficiently small, then the Newton sequence $\{X_{i}\}$ is well defined and $\| P'_{0}[X_{i}]^{-1}\|\leq c\|\tXi\|^{-1}$ for all $i\geq1$ and some constant $c>0$. Moreover,
	\begin{equation}
...
Defining
\begin{align*}
\l N &= \proj_{\N_{\e}} P''[S](N, N),\\
\nu N &= \proj_{\N_{\e}} \tilde{X},\numberthis\\
s &= {\rm sign}(\e\l),
\end{align*}
we consider the conical region
\begin{equation}\label{eq:Wsset}
W_{s}(\rho, \theta) = \left\{ X \left|\,0 < \|\tilde{X}\| < \rho, ~\|\proj_{\M_{\e}}(\tilde{X})\| \leq \theta \|\proj_{\N_{\e}}(\tilde{X})\|, ~{\rm sign}(\nu) = s \right.\right\}.
\end{equation}
...
\begin{theorem}{\rm (cf. \cite[Theorem 3.22]{DeckerKelley1985})}\label{thm:NealySingularConvergence}
	Assume $\l \neq 0$, and $\e > 0$.
	Then, there are continuous functions $\rho = \rho(\e)$, $\theta = \theta(\e)$, monotonically increasing as $\e \downarrow 0$ such that if $X_{0} \in W_{s}(\rho, \theta)$ then $P'_{\e}[X_{0}]^{-1}$ exists and all subsequent Newton iterates remain in this set and converges to $S$. Further,
	\begin{equation}
	\|\proj_{\N_{\e}}\tXii\| < \tfrac{3}{4}\|\proj_{\N_{\e}}\tXi\|, \qquad \|\proj_{\M_{\e}}(\tXii)\| \leq K\|\tXi\|^{2},
	\end{equation}
...
여기서, $P_{\e}$이 $P_{0}$로 접근하는 bifurcation problem이 있는지 {\color{red}보여주냐}, Decker에 의해 존재하니까 {\color{red}그냥 써도 된다}고 주장하느냐의 문제가 있겠다.
분명히 있긴 한데, 왜 그렇게 되는지 보여주지를 못하겠다.
Translation에 의한 ordered pair relation으로 추정되지만, 보여주지 못하면 의미가 없다.
...
\begin{equation}
P_{\e}(X) = (A_{0} - \e S) + (A_{1} + \e I_{m})X + \sum_{k=2}^{n}A_{k}X^{k} = 0,\quad\e \leq \mu
\end{equation}

\begin{align*}
\P'_{\e}[S] &= I_{m} \otimes (A_{1} + \e I_{m}) + \sum_{k=2}^{n}\sum_{l=0}^{k-1}(S^{k-l-1})^{T} \otimes A_{k}S^{l}\\
&= \P'[S] + \e I_{m^2},
\end{align*}

\begin{align*}
	X_{1} &= X_{0} - P'_{\e}[X_{0}]^{-1}(P(X_{0}))\\
	P'_{\e}[X_{0}](H_{0}) &= -P(X_{0})
\end{align*}

\begin{align*}
	& \sum_{k=2}^{n}\sum_{l=0}^{k-1}A_{k}X_{0}^{l}H_{0}X_{0}^{k-l-1} + (A_{1}+\e I_{m})H_{0} \\ 
	&\quad= -A_{0} + \e S - A_{1}X_{0} - \e X_{0} - \sum_{k=2}^{n}A_{k}X_{0}^{k}
\end{align*}

If $X_{0} = 0$,
\begin{align*}
	(A_{1}+\e I_{m})H_{0} &= -A_{0} + \e S \\
	X_{1} - 0 = H_{0} &= -(A_{1}+\e I_{m})^{-1}(A_{0} - \e S) \\
	&= -(A_{1}+\e I_{m})^{-1}A_{0} -(A_{1}+\e I_{m})^{-1}(-\e S).
\end{align*}

If $X_{0} = \e S$,
\begin{align*}
	\sum_{k=2}^{n}\sum_{l=0}^{k-1}& A_{k}(\e S)^{l}H_{0}(\e S)^{k-l-1} + (A_{1}+\e I_{m})H_{0} \\
	&= -A_{0} + \e S - A_{1}(\e S) - \e I_{m}(\e S) - \sum_{k=2}^{n}A_{k}(\e S)^{k} \\
	\sum_{k=2}^{n}\sum_{l=0}^{k-1}& \e^{k-1}A_{k}S^{l}H_{0}S^{k-l-1} + (A_{1}+\e I_{m})H_{0} \\
	&= -A_{0} + \e S - \e A_{1}S - \e^{2}S - \sum_{k=2}^{n}\e^{k}A_{k}S^{k} \\
\end{align*}}
