...
Since the representation of the Fr\'echet derivative of $P_{\mu - \e}$ at $S$ is that
\begin{align*}
...
\begin{equation}
	\begin{array}{l}
		-P'_{\e}[S](E) = \e E, \quad \N_{\e} = {\rm span}(E), \\
		\RR^{\mbym} = \M_{\e} \oplus \N_{\e}, \quad P'_{\e}[S](\M_{\e}) = \M_{\e},
	\end{array}
\end{equation}
where $E = \vec^{-1}({\bf e})$ and ${\bf e}$ is a unit eigenvector of $-\P'[S]$ corresponding to $\mu$.

By \cite{DeckerKelley1985}, $P_{\e}(X) = 0$ is the bifurcation problem where $\e$ is a chosen continuation parameter and the solution arc $S_{\e}$ is a constant arc.

***

Let $P_{\e}(X) = 0$ be the bifurcation problem where $\e$ is a chosen continuation parameter and $\e = 0$ denotes the bifurcation point and let $S_{\e}$ denote a solution arc through the branch point.
We assume that the follwings hold,
\begin{equation}\label{eq:bifurcationAssumption}
	\begin{array}{l}
		\N_{0}={\rm Ker}(P'_{0}[S_{0}])={\rm span}\{E_{0}\},\quad\|E_{0}\| = 1, \\
		P'[S_{0}](\M_{0}) = \M_{0},\qquad \RR^{\mbym} = \M_{0} \oplus \N_{0}.
	\end{array}	
\end{equation}
Then, we can define $\proj_{\N_{0}}$ to be the projection onto $\N_{0}$ parallel to $\M_{0}$ and $\proj_{\M_{0}}=I-\proj_{\N_{0}}$.
Clearly, $P'_{0}[S_{0}]$ is a Fredholm operator of index zero and hence will remain so for $\e$ near zero.
