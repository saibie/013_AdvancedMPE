...
If a matrix $A$ can be written as $rI-B$ with $B \geq 0$ and $r \in \RR$, we call that $A$ is a \textit{$Z$-matrix}.
...
\begin{theorem}\label{thm:ConvergenceNewtonMPE}{\rm\cite[Theorem 2.2]{SeoSeoKim2018}}
	...
	Furthermore, $-\P'_{X_{i}}$ is a nonsingular irreducible $M$-matrix for $i \in \NN$, and $-\mathcal{P}'_{S}$ is an irreducible $M$-matrix.
\end{theorem}
...
\begin{equation}
	\begin{array}{l}
		-P_{S}(E) = \e E, \quad\|E\| = 1, \quad\N={\rm span}\{E\},\\
		\RR^{\mbym} = \M \oplus \N,\qquad P_{S}(\M) = \M.
	\end{array}	
\end{equation}
Then, we can define $\pjn$ to be the projection onto $\N$
parallel to $\M$ and $\pjm=I-\pjn$.
...
\begin{equation}\label{eq:Wsset}
	W_{s}(\rho, \theta) = \left\{ X \left|\,0 < \|\tilde{X}\| < \rho, ~\|\pjm(\tilde{X})\| \leq \theta \|\pjn(\tilde{X})\|, ~{\rm sign}(\mu) = s \right.\right\}.
\end{equation}
