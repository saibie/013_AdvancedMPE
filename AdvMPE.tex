...
\begin{lem}
	There exists a constant $c_{3} > 0$ such that $\|F'[x]y\| \geq c_{3}\|y\|$ holds 
	for all $y \in X$ and $x \in B_{\rho}(s)$.
\end{lem}

\begin{proof}
	\begin{align*}
	F'[x]y &= F'[s]y + F''[s]y(x - s) + \frac12 F'''[s]y(x - s)(x - s) + O(\|y\|\|x - s\|^3)\\
	&= F'[s]y + F''[s]y(x - s) + O(\|y\|\|x - s\|^2)\\
	&= F'[s]y + O(\|y\|\|x - s\|)
	\end{align*}
\end{proof}
...
\begin{proof}
	...
	We finally note that since $F'[x_{i}]m_{i} = F''[s](x_{i} - s)m_{i} + \beta_{2}(x_{i})m_{i}$, then
	\begin{equation}
		c_{1}\|x_{i} - s\|\cdot\|m_{i}\| - c_{6}\|x_{i} - s\|^{2}\|m_{i}\| \leq 1
	\end{equation}
	and
	\begin{equation}
		1 \leq \|F''[s]\|\cdot\|x_{i} - s\|\|m_{i}\| + c_{6}\|x_{i} - s\|^{2}\cdot\|m_{i}\|.
	\end{equation}
	...
	Define $w_{i} = F''[s]P_{N_{1}}(x_{i} - s)m_{i}$.
\end{proof}
