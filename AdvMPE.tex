...
\begin{proof}
	...
	\begin{align*}
		...
		&\geq {\color{red}\left(c_{1} - \frac{\e}{\theta}\right)}\|\pjm(\tilde{X}_{\e,i})\| - c_{2}\|\tilde{X}_{\e,i}\|^{2} \\
...
From Theorem \ref{thm:NealySingularConvergence}, we see that $\pjm(\tilde{X}_{\e,i})$ is terminated more than $\pjn(\tilde{X}_{\e,i})$ for the Newton sequence $\{X_{\e,i}\}$ converging to $S$.
It means that $\{\tilde{X}_{\e,i}\}_{i \geq i_{0}}$ lies on near one-dimensional subspace $\N$ for some $i_{0}$.
So, with just a scalar multiplication, we can accelerate Newton's method.
