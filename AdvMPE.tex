...
\begin{document}
	...
	\begin{align*}
		Q(X) & = A_{2}X^{2} + A_{1}X + A_{0} = 0 \\
		X_{i+1} & = X_{i} - Q_{X_{i}}^{\prime -1}(Q(X_{i})) = X_{i} + H_{i} \\
		& \\
		X_{1} & = X_{0} - Q_{X_{0}}^{\prime -1}(Q(X_{0})) = X_{0} - Q_{X_{0}}^{\prime -1}(A_{0}) = X_{0} + H_{0} \\
		& \\
		Q(X_{1}) & = A_{2}X_{1}^{2} + A_{1}X_{1} + A_{0} \\
		& = A_{2}(X_{0} + H_{0})^{2} + A_{1}(X_{0} + H_{0}) + A_{0} \\
		& = Q(X_{0}) + Q'_{X_{0}}(H_{0}) + A_{2}H_{0}^{2} \\
		& = A_{2}\left(Q_{X_{0}}^{\prime -1}(A_{0})\right)^{2} \\
		X_{2} & = X_{1} - Q_{X_{1}}^{\prime -1}\left(A_{2}\left(Q_{X_{0}}^{\prime -1}(A_{0})\right)^{2}\right) \\
		& \\
		X_{3} & = X_{2} - Q_{X_{2}}^{\prime -1}\left( Q(X_{2}) \right) \\
		X_{3} & = X_{2} - Q_{X_{2}}^{\prime -1}\left(A_{2}H_{1}^{2}\right) \\
		& = X_{2} - Q_{X_{2}}^{\prime -1}\left(A_{2}\left( Q_{X_{1}}^{\prime -1}\left(A_{2}\left(Q_{X_{0}}^{\prime -1}(A_{0})\right)^{2}\right) \right)^{2}\right) \\
		&\\
		H_{i+1} & = -Q_{X_{i+1}}^{\prime -1}(A_{2}H_{i}^{2}) \\
		\Q'_{X_{i}} & = I \otimes (A_{1} + A_{2}X_{i}) + X_{i}^{T} \otimes A_{2} \\
		\Q_{X_{i}}^{\prime -1} & = \left[ I \otimes (A_{1} + A_{2}X_{i}) + X_{i}^{T} \otimes A_{2} \right]^{-1} \\
	\end{align*}
	...
	\begin{align*}
		F'[x]m &= F'[x^*]m + F''[x^*]m(x-x^*) + \frac12 F'''[x^*]m(x-x^*)(x-x^*)+ \cdots\\
		&= F''[x^*]m(x-x^*) + \frac12 F'''[x^*]m(x-x^*)(x-x^*)+ \cdots
	\end{align*}
	
	\begin{align*}
		&\exists c_5, \rho, \theta > 0~;~ \forall x \neq x^* \in W_{\rho,\theta}(x^*), 
		\gamma(F'[x]N_{1}, F'[x]X_{1}) \geq c_5\\
		&\left(C_{\theta}(x^*) = \{x\in X:\|P_{X_{1}}(x-x^*)\|\leq \theta\|P_{N_{1}}(x-x^*)\|\},\right.\\
		&W_{\rho, \theta}(x^*) = B_{\rho}(x^*) \cap C_{\theta}(x^*),\\
		&\left.\gamma(M, N) = \inf\{{\rm dist}(u,N):u\in M, \|u\|=1\}\right)
	\end{align*}
	
	\begin{proof}
		\begin{align*}
			\text{Fix $\rho > 0$.} &\Longrightarrow \text{Suppose the conclusion is false.}\\
			&\Longrightarrow \exists \{x_i\}\in B_{\rho}(x^*), \{m_i\}\in N_1, \{y_i\}\in X_1;\\
			&\qquad \|F'[x_i]m_i\| = 1, \|F'[x_i]m_i - F'[x_i]y_i\|\equiv \e_i \rightarrow 0,\\
			&\qquad \|P_{X_{1}}(x_i - x^*)\| \leq \theta_i\|P_{N_{1}}(x_i - x^*)\| \text{ with }\theta_i \rightarrow 0.\\
			&\Longrightarrow \text{For small }\rho, F'[x]N_{1}\text{:Nontrivial}, x \neq x^* ~\wedge ~\exists a_1, a_2 > 0 ~;~ a_1 \leq \|y_i\| a_2.\\
			&\Longrightarrow F''[x^*]N_{1}N_{1} : \text{1-dim}
		\end{align*}
	\end{proof}
