...
\end{align*}
To calculate $\Phi_{(X_{i}, H_{i})}(k,l)$, we need $\binom{k}{l}(k - 1)$-times of matrix multiplications.
Hence, there are
\begin{equation}
	\left(\sum_{k=l}^{n} \binom{k}{l}(k - 1)\right)(n-s+1) + \left(\sum_{r=s}^{n} \binom{r}{s}(r - 1)\right)(n-l+1)
\end{equation}
of matrix multiplications for given $s$ and $l$.


\section{Note}

\begin{align*}
	&\sum_{l=0}^{n}\sum_{s=0}^{n}\left[\left(\sum_{\substack{k=l\\k\neq 0}}^{n} \binom{k}{l}(k - 1)\right)(n-s+1)\right] + \sum_{l=0}^{n}\sum_{s=0}^{n}\left[\left(\sum_{\substack{r=s\\r \neq 0}}^{n} \binom{r}{s}(r - 1)\right)(n-l+1)\right]\\
	&=\frac{(n+1)(n+2)}{2}\sum_{l=0}^{n}\left(\sum_{\substack{k=l\\k\neq 0}}^{n} \binom{k}{l}(k - 1)\right) + \sum_{l=0}^{n}\sum_{s=0}^{n}\left[\left(\sum_{\substack{r=s\\r \neq 0}}^{n} \binom{r}{s}(r - 1)\right)(n-l+1)\right]\\
	&=\frac{(n+1)(n+2)}{2}\sum_{l=0}^{n}\sum_{\substack{k=l\\k\neq 0}}^{n} \binom{k}{l}(k - 1) + \frac{(n+1)(n+2)}{2}\sum_{s=0}^{n}\sum_{\substack{r=s\\r\neq 0}}^{n} \binom{r}{s}(r - 1)\\
	&=(n+1)(n+2)\sum_{l=0}^{n}\sum_{\substack{k=l\\k\neq 0}}^{n} \binom{k}{l}(k - 1)\\
	&=(n+1)(n+2)\sum_{k=2}^{n}(k - 1) \sum_{l=0}^{k} \binom{k}{l}\\
	&=(n+1)(n+2)\sum_{k=2}^{n}2^{k}(k - 1) \\
	&=(n+1)(n+2)\left( (2n - 3) 2^{n} + 4 \right) \\
	&=(n+1)(n+2)(2n - 3) 2^{n} + 4(n+1)(n+2)
\end{align*}

$\uparrow$ 실험결과 처음부터 재계산 각인데 ㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋ

\begin{align*}
	\Sigma&=\sum_{k=2}^{n}2^{k}(k - 1) = 2^{2} + 2^{3}\cdot 2 + \cdots + 2^{n}(n - 1)\\
	2\Sigma&=\sum_{k=2}^{n}2^{k+1}(k - 1) = 2^{3} + 2^{4}\cdot 2 + \cdots + 2^{n+1}(n -1)\\
	2\Sigma - \Sigma&= 2^{n+1}(n - 1) - 2^{2} - 2^{3} - 2^{4} - \cdots - 2^{n}\\
	\Sigma&= 2^{n+1}(n - 1) - 4\sum_{k=0}^{n-2} 2^{k}\\
	&= 2^{n+1}(n - 1) - 4\frac{2^{n-2} - 1}{2 - 1} = 2^{n+1}(n - 1) - 2^{n} + 4\\
	&= (2n - 3) 2^{n} + 4
\end{align*}

\begin{tabular}{|c|c|c|c|c|c|c|c|}
	\hline
	\backslashbox{$k$}{$l$} & $0$ & $1$ & $2$ & $\cdots$ & $n-2$ & $n-1$ & $n$ \\
	\hline
	$0$ &  &  &  & $\cdots$ &  &  &  \\
	\hline
	$1$ & $\vee$ & $\vee$ &  & $\cdots$ &  &  &  \\
	...
	\hline
	$n-2$ & $\vee$ & $\vee$ & $\vee$ & $\cdots$ & $\vee$ &  &  \\
	\hline
	$n-1$ & $\vee$ & $\vee$ & $\vee$ & $\cdots$ & $\vee$ & $\vee$ &  \\
	\hline
	$n$ & $\vee$ & $\vee$ & $\vee$ & $\cdots$ & $\vee$ & $\vee$ & $\vee$ \\
	\hline
\end{tabular}

\begin{tabular}{|c|c|c|c|c|c|c|c|}
	\hline
	\backslashbox{$s$}{$l$} & $0$ & $1$ & $2$ & $\cdots$ & $n-2$ & $n-1$ & $n$ \\
	\hline
	$0$ & $0$ & $1$ & $2$ & $\cdots$ & $n-2$ & $n-1$ & $n$ \\
	\hline
	$1$ & $1$ & $2$ & $3$ & $\cdots$ & $n-1$ & $n$ & $n+1$ \\
	\hline
	$2$ & $2$ & $3$ & $4$ & $\cdots$ & $n$ & $n+1$ & $n+2$ \\
	\hline
	$\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ & $\ddots$ & $\vdots$ & $\vdots$ & $\vdots$ \\
	\hline
	$n-2$ & $n-2$ & $n-1$ & $n$ & $\cdots$ & $2n-4$ & $2n-3$ & $2n-2$ \\
	\hline
	$n-1$ & $n-1$ & $n$ & $n+1$ & $\cdots$ & $2n-3$ & $2n-2$ & $2n-1$ \\
	\hline
	$n$ & $n$ & $n+1$ & $n+2$ & $\cdots$ & $2n-2$ & $2n-1$ & $2n$ \\
	\hline
\end{tabular}
...
\begin{tabular}{|c|c|c|c|c|c|}
	\hline
	\backslashbox{$r$}{$k$} & $l$ & $l+1$ & $\cdots$ & $n-1$ & $n$ \\
	\hline
	$s$ & $ \binom{s}{s}(s-1) + \binom{l}{l}(l - 1) + 3$ & $ \binom{s}{s}(s-1) + \binom{l+1}{l}l + 3$ & $\cdots$ & $ \binom{s}{s}(s-1) + \binom{n-1}{l}(n-2) + 3$ & $ \binom{s}{s}(s-1) + \binom{n}{l}(n - 1) + 3$ \\
	\hline
	$s+1$ & $ \binom{s+1}{s}s + \binom{l}{l}(l - 1) + 3$ & $ \binom{s+1}{s}s + \binom{l+1}{l}l + 3$ & $\cdots$ & $ \binom{s+1}{s}s + \binom{n-1}{l}(n-2) + 3$ & $ \binom{s+1}{s}s + \binom{n}{l}(n - 1) + 3$ \\
	\hline
	$\vdots$ & $\vdots$ & $\vdots$ & $\ddots$ & $\vdots$ & $\vdots$ \\
	\hline
	$n-1$ & $\binom{n-1}{s}(n-2) + \binom{l}{l}(l - 1) + 3$ & $\binom{n-1}{s}(n-2) + \binom{l+1}{l}l + 3$ & $\cdots$ & $\binom{n-1}{s}(n-2) + \binom{n-1}{l}(n-2) + 3$ & $\binom{n-1}{s}(n-2) + \binom{n}{l}(n - 1) + 3$ \\
	\hline
	$n$ & $\binom{n}{s}(n-1) + \binom{l}{l}(l - 1) + 3$ & $\binom{n}{s}(n-1) + \binom{l+1}{l}l + 3$ & $\cdots$ & $\binom{n}{s}(n-1) + \binom{n}{l}(n-1) + 3$ & $\binom{n}{s}(n-1) + \binom{n}{l}(n - 1) + 3$ \\
	\hline
\end{tabular}}
...
\begin{tabular}{|c|c|c|c|c|c|}
	...
	\backslashbox{$r$}{$k$} & $l$ & $l+1$ & $\cdots$ & $n-1$ & $n$ \\
	\hline
	$s$ & $ (s-1) + (l - 1) + 3$ & $ (s-1) + (l+1)l + 3$ & $\cdots$ & $ (s-1) + \binom{n-1}{l}(n-2) + 3$ & $ (s-1) + \binom{n}{l}(n - 1) + 3$ \\
	\hline
	$s+1$ & $ (s+1)s + (l - 1) + 3$ & $ (s+1)s + (l+1)l + 3$ & $\cdots$ & $ (s+1)s + \binom{n-1}{l}(n-2) + 3$ & $ (s+1)s + \binom{n}{l}(n - 1) + 3$ \\
	\hline
	$\vdots$ & $\vdots$ & $\vdots$ & $\ddots$ & $\vdots$ & $\vdots$ \\
	\hline
	$n-1$ & $\binom{n-1}{s}(n-2) + (l - 1) + 3$ & $\binom{n-1}{s}(n-2) + (l+1)l + 3$ & $\cdots$ & $\binom{n-1}{s}(n-2) + \binom{n-1}{l}(n-2) + 3$ & $\binom{n-1}{s}(n-2) + \binom{n}{l}(n - 1) + 3$ \\
	\hline
	$n$ & $\binom{n}{s}(n-1) + (l - 1) + 3$ & $\binom{n}{s}(n-1) + (l+1)l + 3$ & $\cdots$ & $\binom{n}{s}(n-1) + \binom{n}{l}(n-1) + 3$ & $\binom{n}{s}(n-1) + \binom{n}{l}(n - 1) + 3$ \\
	\hline
\end{tabular}}
